<?php
/**
 * Admin failed order email — Child theme override.
 *
 * Se envía al admin cuando un pedido falla (pago fallido → failed).
...
<p>
	...
	echo wp_kses_post(
		sprintf(
			...
			__( 'Payment for order %s from %s has failed. The order was as follows:', 'woocommerce' ),
			'<a href="' . esc_url( admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ) ) . '">#' . esc_html( $order->get_order_number() ) . '</a>',
